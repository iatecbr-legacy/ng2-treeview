    ...
    template: `
      <div *ngIf="item" class="treeview-item">
          ...
          <div *ngIf="!item.collapsed">
              <treeview-item [config]="config" *ngFor="let child of item.children" [item]="child" [template]="template" (checkedChange)="onChildCheckedChange(child, $event)">
              </treeview-item>
          </div>
      </div>
    `,
...
export class TreeviewItemComponent {
    ...
    onChildCheckedChange(child: TreeviewItem, checked: boolean) {
        if (!this.config.decoupleChildFromParent && !this.config.decoupleChildFromParentForEntity) {
